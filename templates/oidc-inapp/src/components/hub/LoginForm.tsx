...
export function LoginForm({
    ...
    const post = (path: string) => `{{MOUNT}}/interaction/${encodeURIComponent(uid)}/${path}`
    const nieuw = `{{MOUNT}}/aanmelden/${encodeURIComponent(uid)}/nieuw`
    ...
    return (
        <div className='auth-card fade-in'>
            ...
            {step === 'mfa' ? <MfaStep action={post('mfa')} error={Boolean(error)} /> : null}
