...
function setHubScripts(target: string, hub: HubChoice, publiek: number): void {
  ...
  pkg.scripts = {
    ...
    // Het registratietoken terugvinden of vervangen zonder in .env te graven.
    // Je hebt het nodig bij elke volgende app die je op deze hub aansluit.
    "hub:token": "tsx src/token.ts",
    "hub:token:nieuw": "tsx src/token.ts --nieuw",
